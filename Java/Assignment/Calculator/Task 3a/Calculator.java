...
class Calculator{  
    ...
    static Integer flag=0;                             /* 0 means last selected was integer, 1 means operator */
    static Integer clicked=0;                          /* 1 if ENTER was pressed once */
    ...
    private static void addKeyListener()
    {
        jf.addKeyListener(new KeyListener()
        {
            ...
            public void keyPressed(KeyEvent ke) {
                ...
                clicked = 1;
                /* if ENTER is pressed, we need to see which integer key or equal/clear or operator key
                   is chosen which can be detected by seeing which button is GREEN/YELLOW */
                if(keyCode == KeyEvent.VK_ENTER)
                {
                    for(JButton button : keyButtons)
                    {
                        ...
                        {
                            System.out.println("Key " + button.getText() + " pressed");
                    ...
                    if(bclear.getBackground()==Color.YELLOW)
                    {
                        System.out.println("Key " + bclear.getText() + " pressed");
                        EvaluateAns.readInput(bclear.getText());
                        flag = flag^1;
                    }
                    if(beq.getBackground()==Color.YELLOW)
                    {
                        System.out.println("Key " + beq.getText() + " pressed");
